...
**/
	...
	else
	{
		...
		newPtr->LPtr = NULL;
		newPtr->RPtr = NULL;
		if (where == NULL) //prazny strom
		{
			(*RootPtr) = newPtr;
		}
		else // neprazdny strom
		{
			if (where->Cont > Content) // doleva
			{
				where->LPtr = newPtr;
			}
			else // doprava
...
**/
	...
	else // strom existuje
	{
		...
		Leftmost_Preorder(RootPtr, &stackP); // vyhleda nejlevejsi
		tmp = RootPtr;
		while (!SEmptyP(&stackP)) // dokud neni zasobnik prazdny
		{
			tmp = STopPopP(&stackP); // pop
			Leftmost_Preorder(tmp->RPtr, &stackP);
		}
	}
}
...
**/
	...
	else
	{
		...
		while (!SEmptyP(&stackP))
		{
			tmp = STopPopP(&stackP); // save a pop
			SPushP(&stackP, tmp);
			left = STopPopB(&stackB); // save a pop
			if (!left)
			{
				STopPopP(&stackP);
				BTWorkOut(tmp);
...
**/
	...
	else
	{
		...
		do
		{
			...
			{
				if(!SEmptyP(&stackP)) // pokud je zasobnik prazdny
				{
					(*RootPtr) = STopPopP(&stackP); // save a pop
				}
			}
			else // koren existuje
			{
				...
				{
					SPushP(&stackP, (*RootPtr)->RPtr); // save a pop
				}	
				tmp = (*RootPtr);
