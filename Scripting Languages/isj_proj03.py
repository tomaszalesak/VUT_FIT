...
def match_permutations(string, words):
    ...
    matching_perms = list(a for a in perms if a in words) # vase reseni
...
def plur2sing(singular, plural):
    ...
    # inversion using zip
    pl2sg = {s: p for s, p in zip(plural, singular)} # vase reseni
    return pl2sg

def vect2word(word2vect):
    v2w = {tuple(t): tt for t, tt in zip(word2vect.values(), word2vect.keys())} # vase reseni
    return v2w
